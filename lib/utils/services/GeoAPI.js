...
export default class GeoAPI
{
    ...
    /**
     * Obtiene las coordenadas (lat, lng) a partir de un enderezo textual via OSM (Geocoding)
     * @param {string} address El enderezo a ser consultado
     * @returns {Promise<{lat:number, lng:number}>}
     */
    async getCoordinatesFromAddress(address){

        // Codifica el enderezo para ser seguro en la URL
        const encodedAddress = encodeURIComponent(address);

        // La URL de busca del Nominatim. O 'q' es para query
        const url = `${this.baseOSM}/search?q=${encodedAddress}&format=json&addressdetails=1&limit=1`;
        ...
        try{
            ...
            if(data && data.length > 0){
                return {
                    lat: parseFloat(data[0].lat), // Converte a string de latitude para número
                    lng: parseFloat(data[0].lon), // Converte a string de longitude para número (No OSM, é 'lon', não 'lng')
                };
            }
    ...
    {
         const url = `${this.baseOSM}/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=1`;
