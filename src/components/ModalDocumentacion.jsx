...
export default function ModalDocumentacion({ isOpen, tipo, data, onClose, onAddOrUpdate, onDelete }) {
  ...
  const handleSubmit = () => {
    if (tipo === "vacuna" && !formData.tipo_vacuna) {
      alert("Debes ingresar el tipo de vacuna");
      return;
    }
      ...
      <div className="modal-container">
        ...
        <form className="modal-form" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="form-actions flex justify-center mt-4">
            <button type="button" className="btn-modal w-32" onClick={onClose}>Volver</button>
            <button type="button" className="btn-modal w-32" onClick={handleSubmit}>{data ? "Editar" : "Agregar"}</button>
            {data && (
